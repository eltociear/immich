  ...
  async extractExifInfo(job: Job<IAssetUploadedJob>) {
    try {
      ...
      if (newExif.livePhotoCID && !asset.livePhotoVideoId) {
        const motionAsset = await this.assetCore.findLivePhotoMatch(newExif.livePhotoCID, AssetType.VIDEO, asset.id);
        if (motionAsset) {
  ...
  async extractVideoMetadata(job: Job<IAssetUploadedJob>) {
    ...
    try {
      ...
      if (newExif.livePhotoCID) {
        const photoAsset = await this.assetCore.findLivePhotoMatch(newExif.livePhotoCID, AssetType.IMAGE, asset.id);
        if (photoAsset) {
